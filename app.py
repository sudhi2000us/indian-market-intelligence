...
# =========================================================
st.set_page_config(
    page_title="Indian Market Intelligence Pro v2",
    page_icon="📈",
    layout="wide",
...
st.markdown("""
<style>
:root { --bg1:#0a1220; --bg2:#101a2c; --panel:#ffffff; --border:#d9e2ef; --text:#0f172a; --muted:#5b6b85; --green:#16a34a; --red:#dc2626; --amber:#d97706; --blue:#2563eb; }
.stApp { background: radial-gradient(circle at 0% 0%, rgba(37,99,235,0.10), transparent 28%), radial-gradient(circle at 100% 0%, rgba(14,165,233,0.08), transparent 25%), linear-gradient(180deg, #f5f7fb 0%, #eef3f9 100%); }
.block-container { max-width: 1280px !important; padding-top: 0.9rem !important; padding-left: 1rem !important; padding-right: 1rem !important; padding-bottom: 2rem !important; }
h1,h2,h3,h4,h5,h6,p,span,div,label { color: var(--text); }
[data-testid="stSidebar"] { width: 270px !important; min-width: 270px !important; background: linear-gradient(180deg, #0b1730 0%, #0f2142 100%); border-right: 1px solid rgba(255,255,255,0.06); }
[data-testid="stSidebar"] * { color: #eef4ff !important; }
.hero { border-radius: 22px; padding: 22px 22px 18px 22px; background: linear-gradient(135deg, #0f2b59 0%, #123870 45%, #1e3a8a 100%); border: 1px solid rgba(255,255,255,0.08); box-shadow: 0 14px 34px rgba(15,23,42,0.16); margin-bottom: 14px; }
.main-title { font-size: 2.15rem; font-weight: 800; letter-spacing: -0.03em; line-height: 1.1; color: white; }
.subtitle { color: #dbe7ff; margin-top: 0.2rem; margin-bottom: 0.85rem; font-size: 1rem; }
.metric-card { background: var(--panel); border: 1px solid var(--border); border-radius: 18px; padding: 16px; min-height: 132px; box-shadow: 0 8px 24px rgba(15,23,42,0.06); display: flex; flex-direction: column; justify-content: space-between; }
.metric-label { color: #4f6aa3; text-transform: uppercase; font-size: 0.74rem; font-weight: 800; letter-spacing: 0.08em; }
.metric-value { color: var(--text); font-size: 1.68rem; font-weight: 800; margin-top: 6px; }
.metric-sub { color: var(--muted); font-size: 0.89rem; margin-top: 7px; }
.section-title { font-size: 1.08rem; font-weight: 800; margin: 0.25rem 0 0.8rem 0; color: var(--text); }
.note-box { background: var(--panel); border: 1px solid var(--border); border-left: 4px solid var(--blue); border-radius: 16px; padding: 14px 16px; margin-bottom: 12px; box-shadow: 0 8px 20px rgba(15,23,42,0.05); }
.good-box { background: #effaf3; border: 1px solid #ccefd9; border-left: 4px solid var(--green); border-radius: 16px; padding: 14px 16px; }
.warn-box { background: #fff8eb; border: 1px solid #f7dfb0; border-left: 4px solid var(--amber); border-radius: 16px; padding: 14px 16px; }
.badge { display: inline-block; padding: 0.38rem 0.72rem; border-radius: 999px; font-size: 0.78rem; font-weight: 800; margin-right: 8px; }
...
.idea-card { background: var(--panel); border: 1px solid var(--border); border-radius: 18px; padding: 16px; min-height: 250px; box-shadow: 0 8px 22px rgba(15,23,42,0.05); }
.small-muted { color: var(--muted); font-size: 0.87rem; }
hr.soft { border: 0; border-top: 1px solid #e7edf5; margin: 10px 0; }
.stTabs [data-baseweb="tab-list"] { gap: 8px; flex-wrap: nowrap !important; overflow-x: auto; }
.stTabs [data-baseweb="tab"] { white-space: nowrap; border-radius: 999px !important; background: #edf2fa !important; border: 1px solid #dbe5f2 !important; color: #17315f !important; padding: 8px 14px !important; }
[data-testid="stDataFrame"] { border-radius: 14px; overflow: hidden; }
@media (max-width: 1000px) { .block-container { padding-left: 0.8rem !important; padding-right: 0.8rem !important; } .main-title { font-size: 1.55rem; } .metric-value { font-size: 1.35rem; } .metric-card { min-height: 110px; padding: 14px; } [data-testid="stSidebar"] { width: 100% !important; min-width: auto !important; } }
</style>
...
def bias_badge(label: str) -> str:
    if label in ["BULLISH", "TRADABLE", "BUY", "POSITIVE", "BULLISH OPEN"]:
...
def fetch_fundamentals(symbol: str) -> dict:
    out = {"symbol": symbol, "market_cap": np.nan, "trailing_pe": np.nan, "forward_pe": np.nan, "price_to_book": np.nan, "debt_to_equity": np.nan, "roe": np.nan, "roce": np.nan, "profit_margin": np.nan, "revenue_growth": np.nan, "earnings_growth": np.nan, "current_ratio": np.nan, "quick_ratio": np.nan, "next_earnings_date": None, "has_data": False}
    try:
...
def fetch_nse_trading_holidays(year: int | None = None) -> pd.DataFrame:
    ...
    try:
        ...
        for dt_str, desc in re.findall(pattern, text):
            try:
...
def fetch_nse_official_breadth() -> dict:
    try:
        ...
        if m_adv and m_dec:
            ...
            total = adv + dec + unch
            return {"source": "NSE official scrape", "advances": adv, "declines": dec, "unchanged": unch, "adv_pct": round((adv / total) * 100, 2) if total else np.nan, "total": total}
    except Exception:
        pass
    return {}

def compute_universe_breadth(universe: dict) -> dict:
    total = 0; above_ema20 = 0; above_ema50 = 0; strong = 0
    for sym in universe:
        ...
        total += 1
        last = df.iloc[-1]
        c = safe_float(last["Close"]); e20 = safe_float(last["EMA20"]); e50 = safe_float(last["EMA50"]); r14 = safe_float(last["RSI14"])
        if c > e20:
            above_ema20 += 1
        if c > e50:
            above_ema50 += 1
        if c > e20 and c > e50 and r14 > 55:
            strong += 1
    return {"source": "Computed scan breadth", "total": total, "above_ema20": above_ema20, "above_ema50": above_ema50, "strong_count": strong, "adv_pct": round((above_ema20 / total) * 100, 2) if total else 0.0}
...
def fetch_gdelt_news(query: str, max_records: int = 20) -> list[dict]:
    ...
    try:
        ...
        for a in data.get("articles", []):
            title = a.get("title", "").strip(); link = a.get("url", "").strip(); source = a.get("domain", "").strip(); seendate = a.get("seendate", "")
            if title and link:
...
def fetch_rss_feed(url: str, source_name: str) -> list[dict]:
    ...
    try:
        ...
        for item in root.findall(".//item")[:20]:
            title = item.findtext("title", default="").strip(); link = item.findtext("link", default="").strip(); pub_date = item.findtext("pubDate", default="").strip()
            if title and link:
...
def collect_news() -> pd.DataFrame:
    ...
    for bucket, q in gdelt_queries.items():
        for item in fetch_gdelt_news(q, max_records=18):
            rows.append({"bucket": bucket, "title": item["title"], "link": item["link"], "source": item["source"], "pub_date": item["pub_date"], "score": headline_sentiment_score(item["title"]), "sector_tags": ", ".join(sector_impact(item["title"])),})
    for item in fetch_rss_feed(PIB_RSS_URL, "PIB"):
        rows.append({"bucket": "govt_policy", "title": item["title"], "link": item["link"], "source": item["source"], "pub_date": item["pub_date"], "score": headline_sentiment_score(item["title"]), "sector_tags": ", ".join(sector_impact(item["title"])),})
    for item in fetch_rss_feed(PIB_ECONOMY_RSS_URL, "PIB"):
        rows.append({"bucket": "govt_policy", "title": item["title"], "link": item["link"], "source": item["source"], "pub_date": item["pub_date"], "score": headline_sentiment_score(item["title"]), "sector_tags": ", ".join(sector_impact(item["title"])),})
    for item in fetch_rbi_press_releases():
        rows.append({"bucket": "rbi_policy", "title": item["title"], "link": item["link"], "source": item["source"], "pub_date": item["pub_date"], "score": headline_sentiment_score(item["title"]), "sector_tags": ", ".join(sector_impact(item["title"])),})
    for item in fetch_whitehouse_briefings():
        rows.append({"bucket": "whitehouse", "title": item["title"], "link": item["link"], "source": item["source"], "pub_date": item["pub_date"], "score": headline_sentiment_score(item["title"]), "sector_tags": ", ".join(sector_impact(item["title"])),})
    if not rows:
...
def summarize_news_bias(news_df: pd.DataFrame) -> dict:
    ...
    if total_score >= 8: label = "POSITIVE"
    elif total_score <= -8: label = "NEGATIVE"
    else: label = "MIXED"
    ...
    reason_parts = []
    if any(news_df["bucket"].eq("rbi_policy")): reason_parts.append("RBI policy/news active")
    if any(news_df["title"].str.contains("oil|crude|brent|wti", case=False, na=False)): reason_parts.append("oil-sensitive headlines active")
    if any(news_df["title"].str.contains("Trump|White House|tariff", case=False, na=False)): reason_parts.append("U.S. policy headlines active")
    if any(news_df["title"].str.contains("election", case=False, na=False)): reason_parts.append("election-event risk active")
    return {"score": total_score, "label": label, "reason": "; ".join(reason_parts) if reason_parts else "Mixed headline flow", "top_positive": top_positive, "top_negative": top_negative}
...
def overnight_score() -> dict:
    ...
    if not np.isnan(us_avg):
        if us_avg > 0.5:
            score += 2; reasons.append("Wall Street supportive")
        elif us_avg < -0.5:
            score -= 2; reasons.append("Wall Street weak")
    asia_avg = np.nanmean([nikkei.get("chg_pct", np.nan), hsi.get("chg_pct", np.nan), shanghai.get("chg_pct", np.nan)])
    if not np.isnan(asia_avg):
        if asia_avg > 0.4:
            score += 2; reasons.append("Asian markets firm")
        elif asia_avg < -0.4:
            score -= 2; reasons.append("Asian markets weak")
    brent_chg = brent.get("chg_pct", np.nan)
    if not np.isnan(brent_chg):
        if brent_chg > 1.5:
            score -= 2; reasons.append("Brent rising sharply")
        elif brent_chg < -1.0:
            score += 1; reasons.append("Brent easing")
    ...
    if score >= 3: label = "BULLISH OPEN"
    elif score <= -3: label = "WEAK OPEN"
    else: label = "MIXED OPEN"
    return {"label": label, "score": score, "reasons": reasons, "us_avg": us_avg, "asia_avg": asia_avg, "brent_chg": brent_chg, "gift": gift.get("value", np.nan)}
...
def build_gap_plan(regime: dict, overnight: dict, nifty_df: pd.DataFrame) -> str:
    ...
    if gap_pct > 0.75:
        return "Strong positive gap likely. Avoid chasing the first candle. Prefer buy-on-dip after first 15-minute hold." if regime["label"] == "BULLISH" else "Positive gap likely, but regime is not strong enough. Wait for opening-range breakout confirmation."
...
def score_fundamentals(fund: dict) -> dict:
    score = 50; notes = []
    pe = safe_float(fund.get("trailing_pe")); pb = safe_float(fund.get("price_to_book")); dte = safe_float(fund.get("debt_to_equity")); roe = safe_float(fund.get("roe")); roce = safe_float(fund.get("roce")); pm = safe_float(fund.get("profit_margin")); rev_g = safe_float(fund.get("revenue_growth")); earn_g = safe_float(fund.get("earnings_growth")); curr = safe_float(fund.get("current_ratio")); quick = safe_float(fund.get("quick_ratio"))
    if not np.isnan(rev_g):
        if rev_g > 0.10: score += 10; notes.append("good revenue growth")
        elif rev_g < 0: score -= 10; notes.append("revenue shrinking")
    if not np.isnan(earn_g):
        if earn_g > 0.10: score += 12; notes.append("good earnings growth")
        elif earn_g < 0: score -= 12; notes.append("earnings shrinking")
    if not np.isnan(roe):
        if roe > 0.15: score += 8; notes.append("healthy ROE")
        elif roe < 0.08: score -= 6
    if not np.isnan(roce):
        if roce > 0.12: score += 8; notes.append("healthy ROCE")
        elif roce < 0.08: score -= 5
    if not np.isnan(pm):
        if pm > 0.10: score += 6; notes.append("healthy margins")
        elif pm < 0.03: score -= 6
    if not np.isnan(dte):
        if dte < 0.7: score += 8; notes.append("manageable debt")
        elif dte > 1.5: score -= 10; notes.append("high debt")
    if not np.isnan(curr):
        if curr >= 1.2: score += 4
        elif curr < 0.9: score -= 4
    if not np.isnan(quick) and quick >= 0.8: score += 2
    if not np.isnan(pe):
        if pe > 60: score -= 6; notes.append("valuation rich")
        elif 0 < pe < 30: score += 3
    if not np.isnan(pb) and pb > 8: score -= 4
    score = max(0, min(100, score))
    block_long = False
    if (not np.isnan(earn_g) and earn_g < -0.20) and (not np.isnan(dte) and dte > 1.5):
        block_long = True; notes.append("weak fundamentals")
    return {"fund_score": score, "fund_notes": ", ".join(notes[:5]) if notes else "mixed fundamentals", "block_long": block_long}

def earnings_risk_adjustment(fund: dict) -> dict:
    penalty = 0; note = "no near earnings risk"; dt_val = fund.get("next_earnings_date")
    ...
    try:
        ...
        if 0 <= days <= 3: penalty = 12; note = f"earnings in {days} day(s)"
        elif 4 <= days <= 7: penalty = 6; note = f"earnings in {days} day(s)"
        elif days < 0: note = "recent earnings passed"
        else: note = f"earnings later ({days} days)"
    except Exception:
        pass
    return {"earnings_penalty": penalty, "earnings_note": note}
...
def score_market_regime(nifty_df_raw: pd.DataFrame, news_bias: dict, overnight: dict, breadth: dict, vix_df: pd.DataFrame) -> dict:
    ...
    score = 50; reasons = []; last = x.iloc[-1]
    if last["Close"] > last["EMA20"]: score += 10; reasons.append("NIFTY above EMA20")
    else: score -= 10
    if last["EMA20"] > last["EMA50"]: score += 12; reasons.append("EMA20 above EMA50")
    else: score -= 12
    if safe_float(last["RSI14"]) > 55: score += 8; reasons.append("RSI supportive")
    elif safe_float(last["RSI14"]) < 45: score -= 8
    if safe_float(last["MACD_HIST"]) > 0: score += 8; reasons.append("MACD positive")
    else: score -= 8
    breadth_pct = safe_float(breadth.get("adv_pct"), 50)
    if breadth_pct >= 65: score += 12; reasons.append("strong breadth")
    elif breadth_pct >= 55: score += 6; reasons.append("healthy breadth")
    elif breadth_pct <= 40: score -= 10
    if not vix_df.empty:
        try:
            vix_last = safe_float(vix_df["Close"].iloc[-1])
            if vix_last > 22: score -= 8; reasons.append("VIX elevated")
            elif vix_last < 14: score += 4; reasons.append("VIX calm")
        except Exception:
            pass
    ...
    elif score <= 36: label = "BEARISH"
    else: label = "NEUTRAL"
    return {"label": label, "score": round(score, 2), "reason": ", ".join(reasons[:6]) + f" | Overnight: {overnight['label']} | News: {news_bias['label']}"}
...
def trading_day_verdict(regime: dict, overnight: dict, news_bias: dict, vix_df: pd.DataFrame) -> dict:
    ...
    if not vix_df.empty:
        try:
            vix_last = safe_float(vix_df["Close"].iloc[-1])
            if vix_last > 22: score -= 3
            elif vix_last < 14: score += 1
        except Exception:
            pass
    if score >= 6: verdict = "TRADABLE"; message = "Bias is supportive. Focus on strongest setups with discipline."
    elif score <= -6: verdict = "AVOID / VERY SELECTIVE"; message = "Conditions are unstable or weak. Capital protection matters more than activity."
    else: verdict = "SELECTIVE"; message = "Trade only top-quality setups. Avoid low-conviction entries."
...
def previous_day_reasoning(nifty_df: pd.DataFrame, news_df: pd.DataFrame) -> str:
    ...
    last_close = safe_float(nifty_df["Close"].iloc[-1]); prev_close = safe_float(nifty_df["Close"].iloc[-2]); chg = pct_change(last_close, prev_close)
    if pd.isna(chg): return "Unable to compute previous day move."
...
def compute_sector_strength(rank_df: pd.DataFrame) -> pd.DataFrame:
    if rank_df.empty or "sector" not in rank_df.columns:
        return pd.DataFrame()
    grp = rank_df.groupby("sector").agg(avg_score=("score", "mean"), avg_tech=("tech_score", "mean"), avg_fund=("fund_score", "mean"), buy_count=("signal", lambda s: (s == "BUY").sum()), watch_count=("signal", lambda s: (s == "WATCH").sum()), stocks=("symbol", "count")).reset_index()
    grp["sector_score"] = (0.50 * grp["avg_score"] + 0.20 * grp["avg_tech"] + 0.15 * grp["avg_fund"] + 0.15 * (grp["buy_count"] / grp["stocks"] * 100)).round(2)
...
def stock_options_ideas(ideas: pd.DataFrame, regime: dict, vix_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in ideas.head(5).iterrows():
        ...
        rows.append({"symbol": row["symbol"], "name": row["name"], "strategy": "Bullish call idea", "suggested_strike": f"{rounded} CE", "view": "Prefer next liquid expiry; enter only after spot confirms strength", "reason": f"Score {row['score']}, regime {regime['label']}, VIX acceptable"})
...
def score_stock(symbol: str, info: dict, regime_label: str, news_df: pd.DataFrame, sector_rank: dict | None = None) -> dict:
    ...
    d = daily.iloc[-1]
    close_ = safe_float(d["Close"]); atr_ = safe_float(d["ATR14"])
    tech_score = 0; tech_notes = []
    if close_ > safe_float(d["EMA20"]): tech_score += 15; tech_notes.append("above EMA20")
    if close_ > safe_float(d["EMA50"]): tech_score += 15; tech_notes.append("above EMA50")
    if safe_float(d["EMA20"]) > safe_float(d["EMA50"]): tech_score += 10; tech_notes.append("EMA20 > EMA50")
    if close_ > safe_float(d["EMA200"]): tech_score += 6; tech_notes.append("above EMA200")
    rsi_ = safe_float(d["RSI14"])
    if 55 <= rsi_ <= 70: tech_score += 15; tech_notes.append("healthy RSI")
    elif rsi_ > 70: tech_score += 6; tech_notes.append("strong momentum")
    elif rsi_ < 45: tech_score -= 10
    if safe_float(d["MACD_HIST"]) > 0: tech_score += 10; tech_notes.append("MACD positive")
    vol_ratio = safe_float(d["VOL_RATIO"], 1.0)
    if vol_ratio >= 1.2: tech_score += 10; tech_notes.append("volume expansion")
    hh20 = safe_float(d["HH20"])
    if close_ >= hh20 * 0.99: tech_score += 8; tech_notes.append("near breakout")
    ret20 = safe_float(d["RET_20D"])
    if not np.isnan(ret20):
        if 0.03 <= ret20 <= 0.25: tech_score += 6; tech_notes.append("healthy 1M trend")
        elif ret20 < -0.05: tech_score -= 6
    tech_score = max(0, min(100, tech_score))

    news_score = 50; news_hits = 0
    for headline in news_df["title"].head(30).tolist():
        t = headline.lower()
        if symbol.lower() in t or name.lower() in t: news_hits += 1
        if sector.lower() in t: news_hits += 1
        if sector == "Banking" and "rbi" in t: news_hits += 1
        if sector == "IT" and ("ai" in t or "tech" in t or "nasdaq" in t): news_hits += 1
        if sector == "Defence" and ("defence" in t or "war" in t): news_hits += 1
        if sector == "Energy" and ("oil" in t or "energy" in t): news_hits += 1
        if sector == "Infrastructure" and ("capex" in t or "infrastructure" in t or "rail" in t or "power" in t): news_hits += 1
    news_score += min(news_hits * 2.5, 15)
    if regime_label == "BULLISH": news_score += 5
    elif regime_label == "NEUTRAL": news_score += 1
    elif regime_label == "BEARISH": news_score -= 6
    sector_boost = 0
    if sector_rank and sector in sector_rank:
        rank_score = sector_rank[sector]
        if rank_score >= 75: sector_boost = 8
        elif rank_score >= 65: sector_boost = 5
        elif rank_score < 45: sector_boost = -5
    ...
    final_score = (0.45 * tech_score + 0.30 * news_score + 0.25 * fund_eval["fund_score"]) - earn_eval["earnings_penalty"]
    if fund_eval["block_long"]: final_score = min(final_score, 45)
    final_score = round(max(0, min(100, final_score)), 2)
    if fund_eval["block_long"]: signal = "AVOID"
    elif final_score >= 66: signal = "BUY"
    elif final_score >= 52: signal = "WATCH"
    elif final_score >= 45: signal = "EARLY"
    else: signal = "AVOID"

    if math.isnan(atr_) or atr_ <= 0: sl = round(close_ * 0.95, 2)
    else: sl = round(close_ - (1.5 * atr_), 2)
    ...
    if earn_eval["earnings_penalty"] >= 12: horizon = "Avoid fresh swing before results"
    elif profit_pct_2 >= 10: horizon = "2 to 4 weeks"
    else: horizon = "1 to 2 weeks"
    notes = []
    notes.extend(tech_notes[:4])
    notes.append(fund_eval["fund_notes"])
    notes.append(earn_eval["earnings_note"])
    return {"symbol": symbol, "name": name, "sector": sector, "signal": signal, "score": final_score, "tech_score": round(tech_score, 2), "news_score": round(news_score, 2), "fund_score": round(fund_eval["fund_score"], 2), "ltp": round(close_, 2), "target1": target1, "target2": target2, "profit_pct_1": profit_pct_1, "profit_pct_2": profit_pct_2, "sl": sl, "stoploss_pct": stoploss_pct, "rr": rr, "horizon": horizon, "notes": " | ".join(notes[:6])}
...
base_rank_df = pd.DataFrame(base_rows).sort_values(by=["score", "profit_pct_2", "fund_score", "tech_score"], ascending=[False, False, False, False]).reset_index(drop=True)
...
rank_df = pd.DataFrame(final_rows).sort_values(by=["score", "profit_pct_2", "fund_score", "tech_score"], ascending=[False, False, False, False]).reset_index(drop=True)
sector_df = compute_sector_strength(rank_df)

ideas = rank_df[(rank_df["signal"].isin(["BUY", "WATCH", "EARLY"])) & (rank_df["fund_score"] >= max(28, min_fund_score - 18)) & (rank_df["stoploss_pct"] <= max_stoploss + 3) & (rank_df["rr"] >= max(1.2, min_rr - 0.4))].head(max_ideas)
if ideas.empty:
    ideas = rank_df[rank_df["signal"].isin(["BUY", "WATCH", "EARLY"])].head(max_ideas)
if regime["label"] == "BEARISH":
    ideas = ideas.head(0)

high_conviction = rank_df[(rank_df["signal"] == "BUY") & (rank_df["fund_score"] >= 35) & (rank_df["rr"] >= 1.5)].head(6)
watch_candidates = rank_df[(rank_df["signal"].isin(["BUY", "WATCH"])) & (rank_df["fund_score"] >= 30)].head(8)
aggressive_swings = rank_df[(rank_df["signal"].isin(["BUY", "WATCH", "EARLY"])) & (rank_df["fund_score"] >= 25)].head(10)

with tabs[2]:
    st.markdown('<div class="section-title">High Conviction</div>', unsafe_allow_html=True)
    if high_conviction.empty:
        st.caption("No high-conviction names at the moment.")
    else:
        top_cards = st.columns(min(4, len(high_conviction)))
        for col, (_, row) in zip(top_cards, high_conviction.head(4).iterrows()):
            with col:
                st.markdown(f"""
                <div class="idea-card">
    ...
    st.markdown('<div class="section-title">Watch / Pullback Candidates</div>', unsafe_allow_html=True)
    if watch_candidates.empty:
        st.caption("No watch candidates at the moment.")
    else:
        st.dataframe(watch_candidates[["symbol", "name", "sector", "signal", "score", "tech_score", "news_score", "fund_score", "ltp", "target1", "target2", "profit_pct_1", "profit_pct_2", "sl", "stoploss_pct", "rr", "horizon", "notes"]], use_container_width=True, hide_index=True)

    st.markdown('<div class="section-title">Aggressive Swing Ideas</div>', unsafe_allow_html=True)
    if aggressive_swings.empty:
        st.caption("No aggressive swing ideas at the moment.")
    else:
        st.dataframe(aggressive_swings[["symbol", "name", "sector", "signal", "score", "tech_score", "news_score", "fund_score", "ltp", "target1", "target2", "profit_pct_1", "profit_pct_2", "sl", "stoploss_pct", "rr", "horizon", "notes"]], use_container_width=True, hide_index=True)
...
with tabs[3]:
    ...
    if sector_df.empty:
        ...
        st.dataframe(sector_df[["sector", "sector_score", "avg_score", "avg_tech", "avg_fund", "buy_count", "watch_count", "stocks"]], use_container_width=True, hide_index=True)
...
with tabs[5]:
    ...
    watch_df = rank_df[(rank_df["signal"] == "WATCH") & (rank_df["fund_score"] >= 30)].head(12)
    ...
    if watch_df.empty:
        ...
        st.dataframe(watch_df[["symbol", "name", "sector", "score", "tech_score", "news_score", "fund_score", "ltp", "sl", "horizon", "notes"]], use_container_width=True, hide_index=True)
...
with tabs[6]:
    ...
    if not show_news.empty:
        st.dataframe(show_news[["bucket", "source", "title", "score", "sector_tags", "link"]], use_container_width=True, hide_index=True)
